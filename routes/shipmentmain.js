...
async function getShipmentContext(shipmentId) {
  ...
  const deliveries = await pool.request().input('shipmentId', sql.BigInt, shipmentId).query(`SELECT dm.deliveryID, dm.customerID, dm.dueDate, dm.completionDate, dm.deliveryService, dm.picksheetComment, CAST(ISNULL(dm.netWeight, 0) AS decimal(18,3)) AS netWeight, CAST(ISNULL(dm.grossWeight, 0) AS decimal(18,3)) AS grossWeight, CAST(ISNULL(dm.palletCount, 0) AS decimal(18,3)) AS palletCount, CAST(ISNULL(dm.deliveryVolume, 0) AS decimal(18,3)) AS deliveryVolume, d.destinationName, d.destinationStreet, d.destinationCity, d.destinationPostCode, d.destinationCountry, d.destinationEmail FROM Logistics.dbo.ShipmentLink sl INNER JOIN Logistics.dbo.DeliveryMain dm ON dm.deliveryID = sl.deliveryID LEFT JOIN Logistics.dbo.Destinations d ON dm.customerID = d.destinationID WHERE sl.shipmentID = @shipmentId ORDER BY dm.deliveryID ASC`);
...
function buildShipmentSummaryLines(context) {
  ...
  return [`Shipment Ref: ${ref}`, `Destination: ${shipment.destinationName || ''}`, `Address: ${shipment.destinationStreet || ''}, ${shipment.destinationCity || ''}, ${shipment.destinationPostCode || ''}, ${shipment.destinationCountry || ''}`, `Incoterms: ${shipment.incoTerms || ''}`, `Planned Collection: ${shipment.plannedCollection ? new Date(shipment.plannedCollection).toLocaleDateString('en-GB') : ''}`, `Forwarder ID: ${shipment.forwarderID ?? ''}`, `Tracking Number: ${shipment.trackingNumber || ''}`, '', `Deliveries Linked: ${deliveries.length}`, `Pallet Count: ${shipment.palletCount ?? 0}`, `Gross Weight: ${shipment.grossWeight ?? 0}`, `Volume: ${shipment.shipmentVolume ?? 0}`, '', `Pallet Records: ${pallets.length}`];
}
function buildDeliveryPackingLines(delivery, pallets) {
  ...
  if (!pallets.length) return [...lines, 'No pallet details linked to this delivery.'];
  pallets.forEach((pallet, index) => { lines.push(`Pallet ${index + 1}: ${pallet.palletType || 'Pallet'} (#${pallet.palletID})`, `  Finished: ${pallet.palletFinish ? 'Yes' : 'No'}`, `  Size (L/W/H): ${pallet.palletLength || 0} / ${pallet.palletWidth || 0} / ${pallet.palletHeight || 0}`, `  Gross Weight: ${pallet.grossWeight ?? 0}`, `  Volume: ${pallet.palletVolume ?? 0}`, `  Location: ${pallet.palletLocation || ''}`, ''); });
  return lines;
}
...
function buildMimeMessage({ from, to, cc, subject, textBody, attachments }) {
  ...
  for (const attachment of attachments) parts.push('', `--${boundary}`, `Content-Type: application/pdf; name="${attachment.fileName}"`, 'Content-Transfer-Encoding: base64', `Content-Disposition: attachment; filename="${attachment.fileName}"`, '', splitBase64Lines(attachment.content.toString('base64')));
  parts.push('', `--${boundary}--`, '');
  return parts.join('\r\n');
}
async function sendSmtpMessage({ from, to, cc, bcc, message }) {
  const settings = getLogisticsSettings();
  if (!settings.smtpHost || !settings.mailFrom) { const err = new Error('Logistics email is not configured. Set SMTP host and from address in env or config.'); err.statusCode = 503; throw err; }
  const recipients = [...to, ...cc, ...bcc].filter(Boolean);
  if (!recipients.length) { const err = new Error('No email recipients were provided.'); err.statusCode = 400; throw err; }
  const socket = settings.smtpSecure ? tls.connect({ host: settings.smtpHost, port: settings.smtpPort, rejectUnauthorized: false }) : net.createConnection({ host: settings.smtpHost, port: settings.smtpPort });
  socket.setEncoding('utf8');
  await new Promise((resolve, reject) => { socket.once('connect', resolve); socket.once('error', reject); });
  const pending = []; const queued = []; let buffer = ''; let chunkLines = [];
  socket.on('data', data => {
    buffer += data; let newlineIndex = buffer.indexOf('\n');
    while (newlineIndex >= 0) {
      const line = buffer.slice(0, newlineIndex + 1).trimEnd();
      buffer = buffer.slice(newlineIndex + 1);
      if (line) { chunkLines.push(line); if (/^\d{3} /.test(line)) { const response = chunkLines.join('\n'); chunkLines = []; const next = pending.shift(); if (next) next.resolve(response); else queued.push(response); } }
  ...
  const assertCode = (response, allowed) => { const code = Number.parseInt(response.slice(0, 3), 10); if (!allowed.includes(code)) throw new Error(`SMTP error ${code}: ${response}`); };
  ...
  try {
    ...
    if (settings.smtpUser) { await sendCommand('AUTH LOGIN', [334]); await sendCommand(Buffer.from(settings.smtpUser).toString('base64'), [334]); await sendCommand(Buffer.from(settings.smtpPass || '').toString('base64'), [235]); }
...
router.get('/', async (req, res) => {
  try { const pool = await getPool(); const result = await pool.request().query('SELECT * FROM Logistics.dbo.ShipmentMain'); res.json(result.recordset); }
  catch (err) { res.status(500).json({ error: err.message }); }
});

router.post('/create-from-deliveries', async (req, res) => {
  const deliveryIDs = normalizeDeliveryIds(req.body.deliveryIDs);
  if (!deliveryIDs.length) return res.status(400).json({ success: false, error: 'Select at least one delivery before creating a shipment.' });
  const pool = await getPool(); const tx = new sql.Transaction(pool);
  try {
    await tx.begin(sql.ISOLATION_LEVEL.SERIALIZABLE);
    const request = tx.request(); const inClause = createInClause(request, deliveryIDs, 'deliveryId');
    const deliveriesResult = await request.query(`SELECT dm.deliveryID, dm.customerID, dm.dueDate, dm.completionDate, dm.deliveryService, dm.picksheetComment, CAST(ISNULL(dm.netWeight, 0) AS decimal(18,3)) AS netWeight, CAST(ISNULL(dm.grossWeight, 0) AS decimal(18,3)) AS grossWeight, CAST(ISNULL(dm.palletCount, 0) AS decimal(18,3)) AS palletCount, CAST(ISNULL(dm.deliveryVolume, 0) AS decimal(18,3)) AS deliveryVolume, d.destinationName, d.destinationStreet, d.destinationCity, d.destinationPostCode, d.destinationCountry, d.destinationEmail, d.defaultIncoterms FROM Logistics.dbo.DeliveryMain dm LEFT JOIN Logistics.dbo.Destinations d ON d.destinationID = dm.customerID LEFT JOIN Logistics.dbo.ShipmentLink sl ON sl.deliveryID = dm.deliveryID WHERE dm.deliveryID IN (${inClause}) AND dm.completionStatus = 1 AND ISNULL(dm.deliveryCancelled, 0) = 0 AND sl.deliveryID IS NULL ORDER BY dm.deliveryID ASC`);
    const deliveries = deliveriesResult.recordset;
    if (deliveries.length !== deliveryIDs.length) throw new Error('One or more deliveries are no longer available for shipment creation. Please refresh and try again.');
    const customerIds = [...new Set(deliveries.map(row => String(row.customerID)))];
    if (customerIds.length !== 1) throw new Error('Selected deliveries must all belong to the same customer.');
    const first = deliveries[0]; const settings = getLogisticsSettings();
    ...
    for (const deliveryID of deliveryIDs) await tx.request().input('shipmentID', sql.BigInt, shipmentID).input('deliveryID', sql.BigInt, deliveryID).query('INSERT INTO Logistics.dbo.ShipmentLink (shipmentID, deliveryID) VALUES (@shipmentID, @deliveryID)');
    await tx.commit();
    const shipment = await getShipmentById(pool, shipmentID); const folder = getShipmentFolderInfo(shipment);
...
router.post('/:shipmentId/create-folder', async (req, res) => {
  try { const context = await getShipmentContext(req.params.shipmentId); const folder = await ensureShipmentFolder(context.shipment); res.json({ success: true, data: { shipmentRef: folder.shipmentRef, folderPath: folder.shipmentPath } }); }
...
router.post('/:shipmentId/generate-packing-list', async (req, res) => {
  try {
    const context = await getShipmentContext(req.params.shipmentId); const generated = await generateShipmentDocuments(context);
    res.json({ success: true, data: { shipmentRef: generated.shipmentRef, folderPath: generated.folderPath, files: generated.files.map(file => ({ fileName: file.fileName, deliveryID: file.deliveryID, downloadUrl: `/api/shipmentmain/${req.params.shipmentId}/documents/${encodeURIComponent(file.fileName)}` })) } });
  } catch (err) { res.status(err.statusCode || 500).json({ success: false, error: err.message }); }
});
router.get('/:shipmentId/documents/:fileName', async (req, res) => {
  try {
    const context = await getShipmentContext(req.params.shipmentId); const folder = getShipmentFolderInfo(context.shipment); const fileName = path.basename(req.params.fileName || '');
    if (!fileName.toLowerCase().endsWith('.pdf')) return res.status(400).json({ success: false, error: 'Only PDF documents are available.' });
    const target = path.join(folder.shipmentPath, fileName); await fsp.access(target, fs.constants.F_OK); return res.sendFile(target);
  } catch (err) { res.status(err.statusCode || 500).json({ success: false, error: err.message }); }
});
router.post('/:shipmentId/send-collection-email', async (req, res) => {
  try {
    const context = await getShipmentContext(req.params.shipmentId);
    if (!isExWorks(context.shipment.incoTerms)) return res.status(400).json({ success: false, error: 'Collection email is only available for Ex Works shipments.' });
    const destinationEmail = String(context.deliveries[0]?.destinationEmail || '').trim();
    if (!destinationEmail) return res.status(400).json({ success: false, error: 'Destination email is missing for this shipment.' });
...
router.get('/id/:shipmentId', async (req, res) => {
  try { const pool = await getPool(); const result = await pool.request().input('shipmentId', sql.BigInt, req.params.shipmentId).query('SELECT * FROM Logistics.dbo.ShipmentMain WHERE shipmentID = @shipmentId'); res.json(result.recordset); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
router.get('/forwarder/:forwarderId', async (req, res) => {
  try { const pool = await getPool(); const result = await pool.request().input('forwarderId', sql.BigInt, req.params.forwarderId).query('SELECT * FROM Logistics.dbo.ShipmentMain WHERE forwarderID = @forwarderId'); res.json(result.recordset); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
router.get('/destination/:destinationId', async (req, res) => {
  try { const pool = await getPool(); const result = await pool.request().input('destinationId', sql.BigInt, req.params.destinationId).query('SELECT * FROM Logistics.dbo.ShipmentMain WHERE destinationID = @destinationId'); res.json(result.recordset); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
router.get('/daterange', async (req, res) => {
  try { const { dateFrom, dateTo } = req.query; const pool = await getPool(); const result = await pool.request().input('dateFrom', sql.DateTime, new Date(dateFrom)).input('dateTo', sql.DateTime, new Date(dateTo)).query('SELECT * FROM Logistics.dbo.ShipmentMain WHERE plannedCollection BETWEEN @dateFrom AND @dateTo'); res.json(result.recordset); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
